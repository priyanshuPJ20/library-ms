...
const ActiveIssues = () => {
    const activeIssues = demoIssueTransactions.filter(t => t.status === 'issued');

    return (
        <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-6xl mx-auto">
                <h1 className="text-3xl font-bold text-black mb-6">Active Issues</h1>
                ...
                {activeIssues.length === 0 ? (
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-hidden">
                                            ...
                                            <tr key={transaction._id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 text-sm">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                                                        daysLeft < 0 ? 'bg-red-100 text-red-800' : daysLeft <= 3 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                                                    }`}>
                                                        {daysLeft} days
                                                    </span>
                                                </td>
                        ...
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
                            <p className="text-sm text-gray-600">Total Active Issues: <span className="font-semibold text-black">{activeIssues.length}</span></p>
                        </div>
                    </div>
                )}
            </div>
